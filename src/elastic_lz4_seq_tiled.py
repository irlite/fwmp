...
lib_path = os.path.join(
    ...
    "libelastic_kernels_seq_tiled.so"
)
...
lib.update_stress_velocity_seq_tiled.argtypes = [
    _float2,
    _float2,
    _float2,
    _float2,
...
lib.update_stress_velocity_seq_tiled.restype = None
...
def load_segy(path):
    with segyio.open(
        ...
        return np.stack(
            [np.asarray(trace) for trace in f.trace]
        ).T
...
def update_stress_velocity_c(
    ...
    lib.update_stress_velocity_seq_tiled(
        vx,
        vz,
        sxx,
        szz,
...
vp0 = load_segy(vp_path)[::ds, ::ds].astype(
    np.float32
)

vs0 = load_segy(vs_path)[::ds, ::ds].astype(
    np.float32
)

rho0 = load_segy(rho_path)[::ds, ::ds].astype(
    np.float32
)
...
for i in range(pad_right):
    sigma[:, -1 - i] = np.maximum(
        sigma[:, -1 - i],
        60.0 * r[pad_right - 1 - i]
    )
...
mu_loc = np.ascontiguousarray(
    add_halo_2d(mu)
)

lam_loc = np.ascontiguousarray(
    add_halo_2d(lam)
)

lam2mu_loc = np.ascontiguousarray(
    add_halo_2d(lam2mu)
)

inv_rho_loc = np.ascontiguousarray(
    add_halo_2d(inv_rho)
)

damp_loc = np.ascontiguousarray(
    add_halo_2d(damp)
)
...
h5.attrs["tile_z"] = int(
    os.environ.get("FWMP_TILE_Z", "8")
)
h5.attrs["tile_x"] = int(
    os.environ.get("FWMP_TILE_X", "256")
)

update_stress_velocity_c(
    vx,
    vz,
    sxx,
    szz,
...
for it in range(n_iterations):
    ...
    if it % frame_stride == 0:
        local_view = np.ascontiguousarray(
            vz[
                local_i0:local_i1,
                local_j0:local_j1
            ]
